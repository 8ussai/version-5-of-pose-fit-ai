...
from modules.training.squats.dataset import SquatNPZDataset
...
class Trainer:
    def __init__(self):
        ...
        # Dataset (NPZ)
        if not config.PROCESSED_DIR.exists() or len(list(config.PROCESSED_DIR.glob("*.npz"))) == 0:
            raise FileNotFoundError(
                f"❌ No npz found in {config.PROCESSED_DIR}\n"
                f"Run first: python preprocess_videos.py"
            )

        full = SquatNPZDataset(config.PROCESSED_DIR, self.hp, split="train")
        ...
        full_val = SquatNPZDataset(config.PROCESSED_DIR, self.hp, split="val")
    ...
    def plot_curves(self):
        out_png = config.OUTPUTS_DIR / "training_curves.png"
        plt.figure()
        ...
        out_png2 = config.OUTPUTS_DIR / "accuracy_curves.png"
        plt.figure()
    ...
    def fit(self):
        best_path = config.MODELS_DIR / "squat_light_best.pt"
